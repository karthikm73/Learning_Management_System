...
const handleSignIn = async (req, res) => {
    ...
    const accessToken = createAccessToken(tutorData, true)
...
/**
 * @desc get tutor details from token present in access token
 * @route GET /auth/tutor/restore
 * @access public
 */

const restoreTutorDetails = asyncHandler(async (req, res) => {
    if (!req.cookies['accessTokenTutor']) {
        return res.status(200).json({ message: 'access token not found', tutorData: null })
    }

    const tutorData = await tutorService.getTutorFromToken(req.cookies['accessTokenTutor'])
    ...
    return res.status(200).json({ message: tutorData ? 'tutor details found' : 'tutor not found', tutorData })
})
